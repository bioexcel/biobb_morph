...
from biobb_morph.morph.createTZ import *

###
# -------------------------------------------------------------------------------------------------------------------------------------------------------------------


def morph(fileIn, pathSources, pathResults, morph, WCEP, toINP, interpo, fusion, rigid, surfRegCEP, checkFElem, checkHaus, regCEP, nodeDistance, moveTo, movement, plane, reduce_param, CreateTZ, lambdaBetaFile, bcpdCommand, abaqusCommand):
    ...
    if CreateTZ == 1:
        ...
        fileIn = mainProgramTZ(fileIn, nodeDistance, moveTo, movement, plane, reduce_param)
    ...
    #pathSources = 'sources'
    #pathResults = 'results'
    pathTemplate = os.path.join(pathSources, 'template')  # source folder: template mesh point cloud
    ...
    pathInp = os.path.join(pathResults, 'inputs')  # .inp folder: folder that contains the .inp final files
    ...
    fileNodesOnSurfaceMorphed = "nodesOnSurface_Morphed_" + numberIVD + "_" + patient + ".txt"
    ...
    if (rigid == 1 and (morph == 1 or morph == 5)):
        ...
        funcRigidMorphed = defFuncRigid(os.path.join(pathTarget, fileToRigid), os.path.join(pathTemplate, fileTemplateNoBEP), outPutFileMorphedRigid, 1000, line_count_rigid, len(nodeNoBEPTemplate), bcpdCommand)
    ...
    if rigid == 1:
        ...
        if regCEP == 1:
            with open(os.path.join(pathOutToInp, fileTemplateCEPupperRigid), 'w') as mor:
                for inode in nodesCEPupper:
                    mor.write(string.format(*nodeTotTemplate[inode]))
            with open(os.path.join(pathOutToInp, fileTemplateCEPupperUpRigid), 'w') as mor:
                for inode in nodesCEPupper_up:
                    mor.write(string.format(*nodeTotTemplate[inode]))
            with open(os.path.join(pathOutToInp, fileTemplateCEPupperDownRigid), 'w') as mor:
                for inode in nodesCEPupper_down:
                    mor.write(string.format(*nodeTotTemplate[inode]))
                                                    
            with open(os.path.join(pathOutToInp, fileTemplateCEPlowerRigid), 'w') as mor:
                for inode in nodesCEPlower:
                    mor.write(string.format(*nodeTotTemplate[inode]))

            with open(os.path.join(pathOutToInp, fileTemplateCEPlowerUpRigid), 'w') as mor:
                for inode in nodesCEPlower_up:
                    mor.write(string.format(*nodeTotTemplate[inode]))

            with open(os.path.join(pathOutToInp, fileTemplateCEPlowerDownRigid), 'w') as mor:
    ...
    if rigid == 1:
        ...
        if regCEP == 1:
            ...
            templateCEPupper_up = os.path.join(pathOutToInp, fileTemplateCEPupperUpRigid)
            templateCEPupper_down = os.path.join(pathOutToInp, fileTemplateCEPupperDownRigid)
            
            templateCEPlower = os.path.join(pathOutToInp, fileTemplateCEPlowerRigid)
            templateCEPlower_up = os.path.join(pathOutToInp, fileTemplateCEPlowerUpRigid)
            templateCEPlower_down = os.path.join(pathOutToInp, fileTemplateCEPlowerDownRigid)
        ...
        if regCEP == 1:
            ...
            templateCEPupper_up = os.path.join(pathTemplate, fileTemplateCEPupperUpRigid)
            templateCEPupper_down = os.path.join(pathTemplate, fileTemplateCEPupperDownRigid)
            
            templateCEPlower = os.path.join(pathTemplate, fileTemplateCEPlower)
            templateCEPlower_up = os.path.join(pathTemplate, fileTemplateCEPlowerUpRigid)
            templateCEPlower_down = os.path.join(pathTemplate, fileTemplateCEPlowerDownRigid)
    ...
    if surfRegCEP == 1:
        ...
        if regCEP == 1:
            ...
            with open(os.path.join(pathOutToInp, fileNodesOnSurfaceCEPupperUp), 'w') as f:
                write = csv.writer(f, delimiter='\t')
                for inode, node in enumerate(nodesCEPupper_up):
                    write.writerow(nodeTotTemplate[nodesCEPupper_up[inode]])
            with open(os.path.join(pathOutToInp, fileNodesOnSurfaceCEPupperDown), 'w') as f:
                write = csv.writer(f, delimiter='\t')
            ...
            with open(os.path.join(pathOutToInp, fileNodesOnSurfaceCEPlowerUp), 'w') as f:
                write = csv.writer(f, delimiter='\t')
        ...
        funcNodesOnSurfaceAFwoNodesCEP = defFuncNonRigid(os.path.join(pathTarget, fileTargetAF), os.path.join(pathOutToInp, fileNodesOnSurfaceAFwoNodesCEP), outPutFileNodesOnSurfaceAFwoNodesCEP, float(lambdaBeta_01[0]), float(lambdaBeta_01[1]), 1000, line_count_AF, len(nodesOnSurfaceAFwoNodesCEP), bcpdCommand)

        funcNodesOnSurfaceNPwoNodesCEP = defFuncNonRigid(os.path.join(pathTarget, fileTargetNP), os.path.join(pathOutToInp, fileNodesOnSurfaceNPwoNodesCEP), outPutFileNodesOnSurfaceNPwoNodesCEP, float(lambdaBeta_01[2]), float(lambdaBeta_01[3]), 1000, line_count_NP, len(nodesOnSurfaceNPwoNodesCEP), bcpdCommand)

        if regCEP == 1:

            funcNodesOnSurfaceCEPupperUP = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPupper_up), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPupperUp), outPutFileNodesOnSurfaceCEPupperUp, 0.7, 2.0, 1000, line_count_CEPupperUp, len(nodesCEPupper_up), bcpdCommand)
            funcNodesOnSurfaceCEPupperDown = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPupper_down), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPupperDown), outPutFileNodesOnSurfaceCEPupperDown, 0.7, 2.0, 1000, line_count_CEPupperDown, len(nodesCEPupper_down), bcpdCommand)

            funcNodesOnSurfaceCEPlowerUP = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPlower_up), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPlowerUp), outPutFileNodesOnSurfaceCEPlowerUp, 0.7, 2.0, 1000, line_count_CEPlowerUp, len(nodesCEPlower_up), bcpdCommand)
            funcNodesOnSurfaceCEPlowerDown = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPlower_down), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPlowerDown), outPutFileNodesOnSurfaceCEPlowerDown, 0.7, 2.0, 1000, line_count_CEPlowerDown, len(nodesCEPlower_down), bcpdCommand)

            funcNodesOnSurfaceCEPupper = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPupper), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPupper), outPutFileNodesOnSurfaceCEPupper, 1.0, 2.0, 1000, line_count_CEPupper, len(nodesOnSurfaceCEPupper), bcpdCommand)

            funcNodesOnSurfaceCEPlower = defFuncNonRigid(os.path.join(pathTarget, fileTargetCEPlower), os.path.join(pathOutToInp, fileNodesOnSurfaceCEPlower), outPutFileNodesOnSurfaceCEPlower, 1.0, 2.0, 1000, line_count_CEPlower, len(nodesOnSurfaceCEPlower), bcpdCommand)
        ...
        count = 0
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceAFwoNodesCEPE), 'r') as fp:
            reader_f = csv.reader(fp, delimiter='\t')
            for line in reader_f:
                if count < lenTZAF:
                    coordCEPTZ.append(
        ...
        count = 0
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceNPwoNodesCEPE), 'r') as fp:
            reader_f = csv.reader(fp, delimiter='\t')
            for line in reader_f:
                if count >= lenTZNP:
                    coordCEPTZ.append(
        ...
        if (morph == 4 or morph == 5):
            with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceAFwoNodesCEPE), 'w') as f:
                write = csv.writer(f, delimiter='\t')
        ...
        funcNodesOnSurfaceAFwNodesCEP = defFuncNonRigid(os.path.join(pathTarget, fileTargetAF), os.path.join(pathOutToInp, fileNodesOnSurfaceAFwNodesCEP), outPutFileNodesOnSurfaceAFwNodesCEP, float(lambdaBeta_02[0]), float(lambdaBeta_02[1]), 1000, line_count_AF, len(nodesOnSurfaceAFwNodesCEP), bcpdCommand)

        funcNodesOnSurfaceNPwNodesCEP = defFuncNonRigid(os.path.join(pathTarget, fileTargetNP), os.path.join(pathOutToInp, fileNodesOnSurfaceNPwNodesCEP), outPutFileNodesOnSurfaceNPwNodesCEP, float(lambdaBeta_02[2]), float(lambdaBeta_02[3]), 1000, line_count_NP, len(nodesOnSurfaceNPwNodesCEP), bcpdCommand)
        ...
        # AF
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceAFwNodesCEPE), 'r') as fp:
            reader_f = csv.reader(fp, delimiter='\t')
            for line in reader_f:
        ...
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceAFwNodesCEPE), 'w') as f:
            write = csv.writer(f, delimiter='\t')
            for inode, node in enumerate(coordAF):
                write.writerow(coordAF[inode])

        # NP
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceNPwNodesCEPE), 'r') as fp:
            reader_f = csv.reader(fp, delimiter='\t')
            for line in reader_f:
        ...
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceNPwNodesCEPE), 'w') as f:
            write = csv.writer(f, delimiter='\t')
        ...
        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceAFwoCEP_final), 'w') as f:
            write = csv.writer(f, delimiter='\t')
            for inode, node in enumerate(coordCEPtotAF):
                write.writerow(coordCEPtotAF[inode])

        with open(os.path.join(pathOutToInp, outPutFileNodesOnSurfaceNPwoCEP_final), 'w') as f:
            write = csv.writer(f, delimiter='\t')
    ...
    funcAF = defFuncNonRigid(targetAF, templateAF, outPutFileAF, float(lambdaBeta_03[0]), float(lambdaBeta_03[1]), 1000, lenNodesTargetNumberAF, lenNodesTemplateNumberAF, bcpdCommand)

    funcRigidNP = defFuncRigid(targetNP, templateNP, outPutFileNP, 1000, lenNodesTargetNumberNP, lenNodesTemplateNumberNP, bcpdCommand)

    funcNP = defFuncNonRigid(targetNP, os.path.join(pathOutToInp, outPutFileNPE), outPutFileNP, float(lambdaBeta_03[2]), float(lambdaBeta_03[3]), 1000, lenNodesTargetNumberNP, lenNodesTemplateNumberNP, bcpdCommand)
    ...
    funcMorphed = defFuncNonRigid(os.path.join(pathOutToInp, fileMorphed), templateMorphed, outPutFileMorphed, float(lambdaBeta_04[0]), float(lambdaBeta_04[1]), 1000, lenNodeMorphed, lenNodeMorphed, bcpdCommand)
    ...
    if (morph == 3 or morph == 5):
        ...
        if fusion == 0:
            ...
            morphingFunc = subprocess.run(["rm", os.path.join(pathOutToInp, fileMorphed)])
    ...
    if (checkFElem == 1 or checkFElem == 2):
        ...
        lenFailedElem = checkFailedElem(outPutFileFailedElem, outPutFileMorphedIVD, pathCompleted, pathTemplate, numberIVD, patient, gap, abaqusCommand)
        ...
        if checkFElem == 2:
            while (lenFailedElem > 0 and gap < 15):
                ...
                funcMorphed = defFuncNonRigid(os.path.join(pathOutToInp, fileMorphed), templateMorphed, outPutFileMorphed, 3.0 + gap, 3.0, 1000, lenNodeMorphed, lenNodeMorphed, bcpdCommand)

                defNonRigid("Entire IVD", numberIVD, patient, funcMorphed, outPutFileMorphed, pathOutToInp)

                #-----------------------------------------------------------------------
                # creating .inp

                coordCentralNode = ObtainCentroid(outPutFileMorphedE, pathOutToInp, nodeNoBEPTemplate, 71143)

                createInpFile("Morphed", numberIVD, patient, outPutFileMorphedE, outPutFileMorphedIVD,pathOutToInp, pathCompleted, pathInpTemplateMorphed, nodeNoBEPTemplate, stringFormat, coordCentralNode)
    ...
    if checkHaus == 1:
        ...
        totCoordToCheck, totCoordToCheckOrder = readCoordFromFile(os.path.join(pathCompleted, outPutFileMorphedIVD), 57, 83538, ",")
        ...
        with open(os.path.join(pathCompleted, outPutFileEuclidean), 'w') as filenodesOn3dOut:
